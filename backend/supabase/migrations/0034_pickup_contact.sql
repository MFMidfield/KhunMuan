-- 0034 · A pickup order carries a name
-- Plan: docs/plan/04-frontend.md §checkout, docs/plan/01-data-model.md §2
--
-- Until now only a delivery collected the customer's name; a pickup order
-- carried nothing but a point, a slot and a four-character code. That code is
-- what the counter had to call out to a crowd, and it is the one thing on the
-- order the customer has to read off a screen to answer. The name is what staff
-- actually say when a box is ready.
--
-- So `customer_name` is required on both routes now. The phone and the room
-- stay optional for a pickup — the customer is standing in front of the shop,
-- so there is nobody to ring and nowhere to deliver.
--
-- One added branch, in section 2. The rest is 0031's body, restated because
...
begin
  ...
  -- Both routes, staff included. A phone order has a name in it whether or not
  -- the form asked for one, and an order nobody can call out by name is the
  -- same problem at the counter as at the door.
  if coalesce(btrim(p_payload ->> 'customer_name'), '') = '' then
    raise exception 'INVALID_PAYLOAD' using detail = 'customer_name';
  end if;
