...
`app.order_code_key` is a 32-byte hex secret set at the database level and held
in Supabase Vault for production. It never reaches the client. Different per
environment, so codes are not comparable across environments — which is correct.
